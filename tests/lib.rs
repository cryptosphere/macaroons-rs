...
const EMPTY_TAG: [u8; 32] =
  [0xe3,0xd9,0xe0,0x29,0x08,0x52,0x6c,0x4c
...
const EXPECTED_TAG_WITH_FIRST_PARTY_CAVEATS: [u8; 32] =
  [0x19,0x7b,0xac,0x7a,0x04,0x4a,0xf3,0x33
...
fn binary_serialization() {
  let token = example_token().add_caveat(&example_first_party_caveat());
  assert_eq!(example_serialized_with_first_party_caveats(), token.serialize());
}
